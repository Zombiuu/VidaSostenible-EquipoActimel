...
--
-- Servidor: localhost
-- Tiempo de generación: 14-06-2017 a las 12:20:09
...
--
-- Base de datos: `fvs_grupo1`
--
CREATE DATABASE IF NOT EXISTS `fvs_grupo1` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
USE `fvs_grupo1`;
...
CREATE TABLE IF NOT EXISTS `depende` (
  ...
  PRIMARY KEY (`id`),
  KEY `depende_pregunta` (`idPregunta`),
  KEY `depende_respuesta` (`idRespuesta`),
  KEY `fk_depende_textoinformativo` (`idTexto`)
...
CREATE TABLE IF NOT EXISTS `dependencia` (
  ...
  `idPregunta` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_dependencia_idDepende` (`idDepende`),
  KEY `fk_dependencia_idPregunta` (`idPregunta`)
...
CREATE TABLE IF NOT EXISTS `edad` (
  ...
  `idTexto` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_edad_textoinformatico` (`idTexto`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8;
...
DROP TABLE IF EXISTS `espacioCasas`;
CREATE TABLE IF NOT EXISTS `espacioCasas` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `texto` varchar(45) NOT NULL,
  `idTexto` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_espacioCasas_textoinformatico` (`idTexto`)
...
DROP TABLE IF EXISTS `numPersonas`;
CREATE TABLE IF NOT EXISTS `numPersonas` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `texto` varchar(45) NOT NULL,
  `idTexto` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_numPersonas_textoinformatico` (`idTexto`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `persona` (
  ...
  PRIMARY KEY (`id`),
  KEY `persona_paises` (`pais`),
  KEY `persona_ccaa` (`ccaa`),
  KEY `persona_tipoCasa` (`tipoCasa`),
  KEY `persona_m2Casa` (`m2Casa`),
  KEY `persona_numPersonas` (`numPersonas`),
  KEY `persona_ingresos` (`ingresos`),
  KEY `persona_conocimientos` (`conocimiento`),
  KEY `persona_studios` (`estudios`),
  KEY `persona_edad` (`edad`)
...
(37, 73, 2, 1, 1, 2, 2, 1, 1, 3, 'femenino'),
(38, 1, 1, 1, 1, 1, 1, 2, 1, 1, 'femenino'),
(39, 1, 1, 2, 1, 1, 1, 1, 1, 1, 'femenino'),
(40, 73, 12, 1, 1, 1, 1, 1, 1, 4, 'femenino'),
(41, 1, 1, 1, 1, 2, 1, 2, 1, 1, 'femenino'),
(42, 1, 1, 1, 1, 2, 1, 2, 1, 1, 'femenino'),
(43, 73, 14, 1, 2, 2, 1, 2, 1, 2, 'femenino');
...
CREATE TABLE IF NOT EXISTS `pertenece` (
  ...
  `idPregunta` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `pertence_categoria` (`idCategoria`),
  KEY `pertence_pregunta` (`idPregunta`)
...
CREATE TABLE IF NOT EXISTS `pregunta` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_pregunta_tipo` (`tipo`),
  KEY `fk_pregunta_dependencia` (`dependencia`)
) ENGINE=InnoDB AUTO_INCREMENT=24 DEFAULT CHARSET=utf8;
...
INSERT INTO `pregunta` (`id`, `pregunta`, `tipo`, `dependencia`, `disponibilidad`, `orden`, `imagen`) VALUES
(1, '¿Cuántos elementos de calefacción tienes en casa? (cuenta todos los radiadores, estufas, etc.)', 1, NULL, 1, 1, NULL),
(2, '¿Tu sistema de calefacción es central o individual?', 1, 3, 1, 2, NULL),
(3, 'Marca el número de radiadores:', 1, 3, 1, 3, NULL),
(4, 'Si tienes, marca los meses en que tienes encendida la calefacción', 2, 3, 1, 4, NULL),
(5, '¿Tienes una instalación de aire acondicionado?', 1, NULL, 1, 5, NULL),
(6, 'Si tienes, marca los meses en los que utilizas el aire acondicionado', 2, NULL, 1, 6, NULL),
(7, 'Si tienes calefacción y/o aire acondicionado y puedes regular la temperatura a voluntad, ¿Cuál es tu estilo de uso de la climatización?', 4, NULL, 1, 7, 'regular'),
(8, '¿Tienes dobles ventanas, o ventanas con doble cristal?', 1, NULL, 1, 8, NULL),
(9, '¿Tienes algún electrodoméstico con etiqueta energética \"A\" (o superior, A+, A++, A+++)', 1, NULL, 1, 9, NULL),
(10, '¿Qué vehículos usas por lo general para tus desplazamientos? (Marca más de uno si es necesario)', 1, NULL, 1, 10, NULL),
(11, '¿Cuánto tiempo tardas en desplazarte a tu trabajo o tu centro educativo? Si marcaste varias opciones en la pregunta anterior, señala el tiempo que tardas en la opción más rápida)', 1, NULL, 1, 11, NULL),
(12, 'Si tienes coche ¿cómo es de grande?', 4, NULL, 1, 12, 'tamCoche'),
(13, 'Marca esta casilla si tu vehículo es eléctrico o híbrido', 2, NULL, 1, 13, NULL),
(14, 'Marca esta casilla si compraste tu coche hace más de diez años.', 2, NULL, 1, 14, NULL),
(15, '¿Vas solo en tu coche?', 4, NULL, 1, 15, 'compCoche'),
(16, '¿Con qué frecuencia haces la compra?', 4, NULL, 1, 16, 'frecCompra'),
(17, '¿Cuál es tu relación con las plantas?', 1, NULL, 1, 17, NULL),
(18, 'Si tienes muchas plantas para regar, ¿empleas algún sistema de riego economizador? (es decir, riego por goteo, hidromacetas, temporizador, etc.)', 1, NULL, 1, 18, NULL),
(19, '¿Cuántos elementos economizadores de agua hay instalados en la fontanería de tu casa? (doble tecla en WC, botón de parada a voluntad, perlizadores o aeradores en grifos, etc.)', 1, NULL, 1, 19, NULL),
(20, '¿Intentas evitar la compra de productos marcados con estas indicaciones?', 1, NULL, 1, 20, NULL),
(21, '¿Cuál es tu relación con la carne?', 4, NULL, 1, 21, 'relCarne'),
(22, '¿Hay alguna de estas cosas en tu domicilio?\r\nUn sitio para guardar el papel y cartón desechado\r\nUn cubo para botellas y frascos de vidrio vacíos\r\nUn cubo o bolsa para meter las latas, bricks y envases de plástico usados\r\nUn lugar para medicamentos, etc', 1, NULL, 1, 22, NULL),
(23, '¿Cómo sueles comprar los alimentos?', 4, NULL, 1, 23, 'compAlimentos');
...
CREATE TABLE IF NOT EXISTS `responde` (
  ...
  `idRespuesta` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `responde_persona` (`idPersona`),
  KEY `responde_respuesta` (`idRespuesta`)
) ENGINE=InnoDB AUTO_INCREMENT=241 DEFAULT CHARSET=utf8;
...
DROP TABLE IF EXISTS `textoInformativo`;
CREATE TABLE IF NOT EXISTS `textoInformativo` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
DROP TABLE IF EXISTS `tipoCasa`;
CREATE TABLE IF NOT EXISTS `tipoCasa` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `texto` varchar(100) NOT NULL,
  `idTexto` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_tipoCasa_textoinformatico` (`idTexto`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8;
